...
describe('#decryptOrStripResponseAttributes', () => {
  ...
  it('strips encrypted attributes except for ones with `dangerouslyExposeValue` set to `true` if type is registered', async () => {
    const { extension } = setup();
...
describe('#encryptAttributes', () => {
  it('encrypts attributes if the type is registered', async () => {
    const { extension } = setup();
